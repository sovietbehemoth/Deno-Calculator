...
class Calculator {
  ...
  private lex_parenthesis(input:string): Array<matches> {
    ...
    if (lcount !== rcount) {
      if (lcount < rcount) {
        //Catches cases of unpaired right parenthesis. Ex: )
        throw new Error(`Syntax, ')' instances expect '('.`);
      } else {
        //Catches cases of unpaired left parenthesis. Ex: (
        throw new Error(`Syntax, '(' instances expect ')'.`);
      }
    }
  ...
  private conv_int(input:any): any {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (typeof (input[i]) === "object") {
        ...
        //The exemption is created in the following cases. The exemption allows the character to be handled
        //as if it were a regular number although the function checks for E.
        //1. A number is entered in scientific notation.
        //2. A number has a trailing E.
        ...
        if (!isNaN(conv) || exemption) { //Is node a number?
          ...
          for (j = i; j < input.length; j++) {
            ...
            //Allows decimals and excludes objects and operators.
            if ((!isNaN(parseFloat(input[j])) || input[j] === "."  || input[j] === "e") && !this.isoperator(input[j]) && typeof(input[j]) !== "object") {
              ...
              //Handles cases where numbers are entered in scientific notation.
              if (input[j] === "e" && input[j+1] === "+") {
                convstr += "e+";
                j++;
              } else {
                //This is where nearly all number building occurs.
                convstr += input[j];
              }
              
              
              if (exemption) {
                //Reset exemption status.
                exemption = false;
              }
          ...
          if (convstr === "e") {
            int_a.push(Math.E);
          } else int_a.push(parseFloat(convstr)); 
          ...
          //The base string to be pushed later.
          let nstr = "";
          ...
          if (nstr.length > 0) {
            ...
            //These are variable constants, they are evaluated on parsing time. They used to be evaluated at the same
            //time as functions but they are essentially like C #define macros and have no dynamic behavior.
            switch (nstr) {
              case "pi":
                int_a.push(Math.PI);
                break
          ...
          if (isop) int_a.push(input[j]); //Regular concat.
  ...
  private eval_vars(input:any): any {
    ...
    const qasserts = (fn:string, i:number, args:string[], optional:boolean=false) => {
      ...
      //arguments, even when arguments are expected. Usually catches instances of zero
      //arguments given when it is a single expression. Ex: cos
      ...
      //The two types of arguments. Nested refers to an explicit function call i.e 'cos(16)' or the function followed by
      //an object. Inferred refers to when the arguments are not contained within an object and instead inferred by\
      //the interpreter based on the amount of arguments expected i.e sqrt 16.
      let iter:any;
      ...
      //Catches most insufficient argument instances where there are more than one argument expected. The
      //prior check catches most function errors. Ex: random(16) when random expects 2 arguments.
      ...
      //Evaluates each argument as separate expression.
      for (let j = 0; j < eval_args.length && j < args.length; j++) {
        ...
        if (eval_string.trim() === "" && !optional) {
          ...
          //Inferred arguments. Ex: random 0, 10
      ...
      //This return reinforces the functions nature as a callback function.
      return [
    ...
    for (let i = 0; i < input.length; i++) {
      if (typeof input[i] === "object") {
      ...
      } else if (!this.isoperator(input[i]) && typeof input[i] !== "number") {  //This will only be a function call.
        ...
        switch(input[i].toLowerCase()) {
          ...
          case "sqrt": {
            ...
            //Catches instances of a signed number being provided.
            if (arg < 0) {
          ...
          case "logbase": {
            ...
            //Special requirement for 2 argument function. Maybe remove, callback almost certainly catches this.
            if (isNaN(num)) {
              throw new Error(`Syntax, 'logBASE' requires 'base,number', found only 'base'.`);
            }

            if (base === 1) {
              throw new Error("Error, Log base 1.");      //Catches a log base of 1 which is illogical.
            } else if (base < 0) {
              throw new Error("Error, Signed log base."); //Catches a negative log base which is also illogical.
            }
          ...
          //Exit is used in the CLI program. it may not be wanted if this class is to be used 
          //as a deno library.
          case "exit": {
          ...
          default:
            //Catches instance of unrecognized functions or identifiers.
  ...
  private evaluate(input:any): any {
    ...
    const value_asserts = (i:number, operator:string) => {
      ...
      //This should probably never happen. The current object is established to exist.
      if (expr[i] === undefined) {
        throw new Error(`Syntax, expected operator around char ${i}.`);
      }
      
      //Catches instance of an operator being at the last char.
      ...
      if (this.isoperator(expr[i-1]) || this.isoperator(expr[i+1])) {
        ...
        //Catches instance of operators illegally being used together. Ex: 1++1
        throw new Error(`Syntax, unexpected operator '${bad_op}' at ${bad_pos}.`);
      }
    }

    //Catches instance of operator being used without a left value.
    const pre_asserts = (op:string, i:number) => {
      if (this.isoperator(expr[i])) {
        throw new Error(`Syntax, expected lvalue for operator '${op}' around char ${i}.`);
      }
    } 
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      } else if (typeof(expr[i]) === "number" && typeof(expr[i+1]) === "number") {
        paren.push(expr[i] * expr[i+1]);
        i++;
    ...
    if (expr.includes("-")) {
      ...
      for (let i = 0; i < expr.length; i++) {
        if ((this.isoperator(expr[i-1]) || i-1 < 0) && expr[i] === "-") {
          ...
          //The negative handler doesn't use the value asserts function so we have to implement
          //custom value asserts to account for it's different behavior.

          //Catches operator compounding. Ex: 1+-5
          if (typeof (expr[i+1]) !== "number") { 
            throw new Error("Syntax, cannot sign an operator.");
          }

          //Catches instance of a sign being applied to zero. Ex: -0
          if (expr[i+1] === 0) {
    ...
    if (expr.length > 1 || this.isoperator(expr[0])) {
      ...
      //is fully evaluated. However this can't be done with the comma operator as it is invalid at this point.
      for (let i = 0; i < expr.length; i++) {


        if (this.isoperator(expr[i])) {
          //Catches cases of the comma operator being used out of the context of a function call.
          if (expr[i] === ",") {
            throw new Error(`Syntax, unexpected use of ',' operator.`);
      ...
      //that is handled here by recursively calling the calculate method on the remaining operands. This area should never
      //actually evaluate. This is just a fail safe in case recursion depth is so convoluted that the parser fails to
      //account for every single token, in this case instead of exiting we work with what we have even at the cost
      //of violating the order of operations.
  ...
  public calculate(input:string, debug=false): number {
    ...
    if (!isFinite(ans)) {
      //Handle Infinity.
      throw new Error("Integer overflow (number evaluated too large).");
    }
